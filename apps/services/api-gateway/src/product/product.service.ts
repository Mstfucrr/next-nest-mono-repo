...
export class ProductService {
  ...
  async findAll(): Promise<ProductEntity[]> {
    return firstValueFrom<ProductEntity[]>(this.productClient.send({ cmd: 'product-find-all' }, {}))
  }

  async findOne(id: string): Promise<ProductEntity | null> {
    return firstValueFrom<ProductEntity | null>(this.productClient.send({ cmd: 'product-find-one' }, { id }))
  }

  async update(id: string, dto: UpdateProductPayload): Promise<ProductEntity | null> {
    return firstValueFrom<ProductEntity | null>(this.productClient.send({ cmd: 'product-update' }, { id, data: dto }))
  }

  async remove(id: string): Promise<boolean> {
    return firstValueFrom<boolean>(this.productClient.send({ cmd: 'product-remove' }, { id }))
  }
}
